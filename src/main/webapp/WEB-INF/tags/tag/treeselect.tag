...
<%@ attribute name="xshuxing" type="java.lang.String" required="false" description="自定义属性"%>
<div class="input-append">
	<input id="${id}Id" name="${name}" class="${cssClass}" type="hidden" value="${value}"${disabled eq 'true' ? ' disabled=\'disabled\'' : ''}  xshuxing="${xshuxing}"/>
		...
		top.$.jBox.open("iframe:${ctx}/tag/treeselect?url="+encodeURIComponent("${url}")+"&module=${module}&checked=${checked}&extId=${extId}&nodesLevel=${nodesLevel}&selectIds="+$("#${id}Id").val(), "&gt;&gt;选择${title}", 300, 420, {
			buttons:{"确定":"ok", ${allowClear?"\"清除\":\"clear\", ":""}"关闭":true}, submit:function(v, h, f){
				if (v=="ok"){
					...
					$("#${id}Id").val(ids);
					$("#${id}Name").val(names);
					if ($.isFunction(window.treeSelectCallBack))
					treeSelectCallBack("${id}",ids,names);
				}//<c:if test="${allowClear}">
				else if (v=="clear"){
					$("#${id}Id").val("");
					$("#${id}Name").val("");
					if ($.isFunction(window.treeSelectCallBack))
						treeSelectCallBack("${id}","","");
                }//</c:if>
			}, loaded:function(h){
